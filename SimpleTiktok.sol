...
contract SimpleTikTok {
    ...
    function postVideo(string memory _url) public {
        ...
        // Use revert for a custom check that prevents posting a duplicate URL.
        for (uint i = 1; i <= videoCount; i++) {
            if (keccak256(bytes(videos[i].url)) == keccak256(bytes(_url))) {
                revert("This video URL has already been posted.");
            }
        }
